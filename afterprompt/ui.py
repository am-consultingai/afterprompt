...
allowlist of read-only endpoints plus a few small writes (checklist ticks, statuses, settings, the heartbeat).
        ...
        if path == "/api/status":
            ...
            if data is None:
                ...
                self._send(200, {"findings": data, "checklist": st.checklist(), "statuses": st.statuses(),
                                 "openable": reveal.openable(data, home=st.home)})
    ...
    def do_POST(self):
        ...
        if path == "/api/heartbeat":
            ...
            if not exposures.valid_hash(h):
                ...
                return
            ok, why = reveal.reveal(st.findings() or {}, h, body.get("index"), home=st.home)
            if ok:
