...
def read_parts(cfg):
    out = ""
    for p in sorted(glob.glob(cfg.w("extracted", "cursor", "*.txt"))):
...
class CursorTests(TempDirTest):
    ...
    def test_wal_content(self):  # U-CUR-2
        ...
        cursor.extract(self.cfg, self.srcs(db))
        con.close()
    ...
    def test_unreadable_db(self):  # U-CUR-4
        ...
        os.chmod(bad, 0)
        info = cursor.extract(self.cfg, self.srcs(bad, good))
        self.assertEqual(info["ok"], 1)
    ...
    def test_part_split(self):  # U-CUR-5
        ...
        cursor.extract(self.cfg, self.srcs(db), part_cap=1000)
        self.assertGreater(len(glob.glob(self.cfg.w("extracted", "cursor", "000_*.txt"))), 2)
